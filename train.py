...
class Train:
    def __init__(self,root):
        ...
        img_top=img_top.resize((1405,280),Image.ANTIALIAS)
        self.photoimg_top=ImageTk.PhotoImage(img_top)
        
        f_lbl=Label(self.root,image=self.photoimg_top)
        f_lbl.place(x=0,y=55,width=1405,height=280) 
        ...
        b1_1.place(x=0,y=335,width=1405,height=50)
        
        img_bottom=Image.open(r"college_images\opencv_face_reco_more_data.jpg")
        img_bottom=img_bottom.resize((1405,280),Image.ANTIALIAS)
        self.photoimg_bottom=ImageTk.PhotoImage(img_bottom)
        
        f_lbl=Label(self.root,image=self.photoimg_bottom)
        f_lbl.place(x=0,y=380,width=1405,height=280) 
        
    
    def train_classifier(self):
        data_dir=("data")
        path = [os.path.join(data_dir, file) for file in sorted(os.listdir(data_dir), key=lambda x: int(x.split('.')[1]))]
